...
//!
//! 所以台账以「包标识」为键逐包记账（**本项目的包没有 ResourceId，键 = 描述@到期毫秒**，
//! 见 [`crate::checkin::parse_packages`]），且每个包的数值**只取观测到的最大值**——
...
pub struct PkgView {
    /// 额度包唯一标识（本项目无 ResourceId，键 = `描述@到期毫秒`，见 [`crate::checkin::parse_packages`]）
    pub key: String,
    pub name: String,
    /// 本周期授予（`credits_limit`）
    pub size: f64,
