...
//! 抄过来。**解绑只摘本机绑定、不删云端那一池** —— 同一串 uuid 可能还绑着别的机器，
//! 删池会让它们取不到凭证；本机解绑时会把「与云端一致」的账号从本地删掉，token 在本机停用。
//!
...
/// 进程启动时加载一次。**只在 `lib.rs` 的 setup 里调**（必须在调度线程与反代起来之前）。
...
/// 这台机器绑定到某一池了吗？**续签路径的分岔判据**（见 `renew.rs` 与 `commands.rs`）。
pub fn bound() -> bool {
    snapshot().0.is_some()
}
...
pub struct BrokerStatus {
    ...
    pub syncing: bool,
    /// 最近一次解绑从本机删除的「与云端一致」账号数（平时为 0，只用于解绑结果反馈）
    #[serde(default)]
...
fn on_failure(kind: FailKind, message: String) -> String {
    match kind {
        FailKind::Gone => {
            set_uuid(None);
            "云端那一池已经不存在了（可能已被服务端清理或在别处删除），本机已退回单机运行，请重新上传或绑定".to_string()
        }
...
fn upload_guard(already_bound: bool) -> Result<(), String> {
    if already_bound {
        return Err(
            "本机已经绑定了云端凭证池。要重新上传，请先解绑（解绑只摘本机绑定，云端那一池不动）。"
                .to_string(),
        );
    }
    Ok(())
...
pub async fn link(dir: &Path, raw_uuid: &str) -> Result<PoolOp, String> {
    ...
    let v = match request(reqwest::Method::GET, &format!("/v1/pool/{uuid}"), None).await {
        ...
        Err((FailKind::Gone, _)) => {
            return Err(
                "云端没有这一池：uuid 可能抄错了，或者它已在别的机器上被解绑。\
                 本机的绑定没有变动。"
                    .to_string(),
            )
        }
...
/// 解绑：摘掉本地 uuid，并把本机「与云端一致」的账号从本地删除。
///
/// **不删云端那一池**：同一串 uuid 可能还绑着别的机器，删池会让它们取不到凭证。
/// 「与云端一致」= 云端池里存在同 key（手机号 → 昵称 → 本地 id，见 [`item_key_of`]）的条目：
/// 这些 token 是本机传上去的，解绑后本机不再持有，token 在本机停用；云端那份留给
/// 仍在绑定的机器。
///
/// 云端已不存在（Gone）或读不到时拿不到对照表，就只清绑定、不动本地账号。
pub async fn unbind(dir: &Path) -> Result<BrokerStatus, String> {
    let Some(uuid) = uuid() else {
        return Ok(status());
    };
    // 拉一份云端条目做对照：把本机与云端一致的账号从本地删掉。删池的活不归解绑管，
    // 读不到对照算「无对照」，不是失败 —— 解绑本身不依赖云端。
    let cloud_keys: Option<std::collections::HashSet<String>> = match request(
        reqwest::Method::GET,
        &format!("/v1/pool/{uuid}"),
        None,
    )
    .await
    {
        Ok(v) => v
            .get("items")
            .cloned()
            .and_then(|x| serde_json::from_value::<Vec<PoolItem>>(x).ok())
            .map(|items| items.iter().map(|i| i.key.clone()).collect()),
        Err(_) => None,
    };
    let removed_local = if let Some(keys) = cloud_keys {
        let list = crate::accounts::load_accounts(dir);
        let before = list.len();
        let keep: Vec<crate::accounts::Account> = list
            .into_iter()
            .filter(|a| !keys.contains(&item_key_of(a)))
            .collect();
        let removed = before - keep.len();
        if removed > 0 {
            let _ = crate::accounts::save_accounts(dir, &keep);
        }
        removed
    } else {
        0
    ...
    let mut st = status();
    st.removed_local = removed_local;
    Ok(st)
}
...
mod tests {
    ...
    fn never_lets_an_older_copy_win() {
        ...
        a.refresh_expires_at = Some(9_001);
        ...
        assert_eq!(a.refresh_expires_at, Some(9_001));
    }
    ...
    fn copies_only_the_gaps_when_not_newer() {
        ...
        assert_eq!(a.refresh_expires_at, Some(1_001));
    }
    ...
    fn merge_round_trips_through_item_and_back() {
        let original = {
            ...
            a.refresh_expires_at = Some(1_770_000_000_000);
        ...
        assert_eq!(born.refresh_expires_at, Some(1_770_000_000_000));
    ...
    /// 真实接口冒烟：上传一池 → 整池同步 → 解绑（解绑不删云端池，测试池留在云端，需另行清理）。
    ///
    ...
    async fn smoke_real_broker_roundtrip() {
        ...
        // 有效期放到 60 天后：`needs_renew` 会判成「不用续」，于是这一趟只验证协议，
        ...
        fake.refresh_expires_at = Some(far_future);
        ...
        unbind(&dir).await.expect("解绑失败");
        assert!(!bound());
        println!("[冒烟] 已解绑（云端池保留，本机已摘绑定）");
