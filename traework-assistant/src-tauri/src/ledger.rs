...
//! # 所以改成读接口的**累计量**
//!
//! 本项目的积分数据源是 `ide_user_ent_usage`（`POST /trae/api/v2/pay/ide_user_ent_usage`，
//! 见 [`crate::checkin`]）。每个额度包带两组数：**授予量**（`entitlement_base_info.quota.credits_limit`）
//! 与**已用量**（`usage.credits_amount`）。这两组数都是「装机以来 / 本周期」的累计量，
//! 对它们做差值即可得到「这段时间消耗 / 新增了多少」—— 与参考项目（billing 的
//! `get-user-resource`，`CycleCapacitySize/Used`）是同一套记账原理，只是字段名不同。
//!
//! 累计量的差值**天然覆盖多客户端**：谁消耗的都会让已用量上涨，不需要知道是谁、
//! 也不需要按请求归属，因此并发不会算错。这与「抓余额」的区别是本质的——
//! 计数器只增不减（周期内），消耗与新增各自留痕，不会互相抵消。
//!
//! # 这里同时也是「账号还剩多少积分」的事实来源之一
//!
//! 每一次采样（简报每小时）都会把这一轮读到的**剩余积分、读数时刻**写进同一个账号
//! 条目（见 [`observe`]）。简报条目里的「剩余」余额就来自这里 —— 它与账户管理页
//! 展示的快照（`credit_snapshot`）取自同一个接口、同一轮响应，只是各自落各自的那一份。
//!
//! # 台账为什么按「包」存
//!
//! 直接对「全部包的合计」取差值会踩两个坑：
//!
//! - **包会过期消失**。查询按到期时间过滤，包一过期就不再出现在响应里，
//!   合计值会**掉下来**，简报立刻变成负数消耗。
//! - **周期会重置**。同一额度包跨周期后已用量可能归零。
//!
//! 所以台账以「包标识」为键逐包记账（键 = `entitlement_id`，见
//! [`crate::checkin::parse_packages`]），且每个包的数值**只取观测到的最大值**——
//! 包过期后条目留在台账里（不再更新），合计因此永不回退。
//!
//! ⚠️ **「逐包」的前提是键真的逐包唯一**：同一批「老用户福利」里就有描述与到期
//! 完全相同的两个包（`product_id` 208 / 209），一旦拿「描述@到期」当键，它们会并成
//! 一条，而**取最大值**会把这条钉在那个已经用满的包上 —— 另一个仍在消耗的包的增量
//! 永远记不进来（真机症状：余额在掉、简报一条不涨）。见 [`PkgView::key`]。
//!
...
/// 一个额度包的观测值 —— 台账的输入。
///
/// 由 `checkin` 从 `ide_user_ent_usage` 的响应里解析出来；定义在这里是因为它就是
/// 台账的输入契约（`checkin` 负责解析，`ledger` 负责记账，两边不互相依赖内部结构）。
///
/// `size` / `used` 取的是**授予量 / 已用量**（`credits_limit` / `credits_amount`），
/// 两者都是累计量（只增不减），差值即「这段时间消耗 / 新增」。
#[derive(Clone, Debug, PartialEq)]
pub struct PkgView {
    /// 额度包唯一标识（`entitlement_id`，见 [`crate::checkin::parse_packages`]）。
    ///
    /// ⚠️ **必须逐包唯一**，否则同名的几个包会合并成一条：`size`/`used` 取最大值后，
    /// 那个**已经用满**的包会把键钉死在它的 `used` 上，另一个还在消耗的包再怎么涨
    /// 也超不过它 ⇒ 增量恒为 0，简报再也固化不出条目（见 [`merge_pkgs`]）。
    pub key: String,
    pub name: String,
    /// 本周期授予（`credits_limit`）
    pub size: f64,
    /// 本周期已用（`credits_amount`）
    pub used: f64,
    /// 当前周期起点（本项目无周期概念，恒为空串 —— 也意味着「翻周期归档」分支不会触发）
    pub cycle_start: String,
    /// 到期时间（毫秒）——**只用于展示**（账号页的资源包列表），不参与记账。
    pub expiry_ms: Option<i64>,
}

/// 台账里一个额度包的条目。数值只增不减（见模块头注释）。
...
pub struct AcctLedger {
    ...
    /// 最近一次拉取到的**剩余积分**（采样路径写这里）。
    ///
    /// 这是简报条目里「余额」的来源：`None` = 还没读到过（界面显示「—」，绝不谎报 0）。
    #[serde(default)]
    pub credits: Option<f64>,
    /// 上面这个读数的时刻（本地 `%Y-%m-%d %H:%M:%S`）。
    /// 简报只把**与被固化小时同一小时**的读数写进条目
    /// （否则就是拿此刻的余额去假装当时）。
    #[serde(default)]
...
pub fn observe(
    ...
    if !led.seeded || mode == Mode::Baseline {
        ...
        if d_used > 0.0 {
            led.hours_used.entry(date.clone()).or_insert([0.0; 24])[hour as usize] += d_used;
        }
        if d_granted > 0.0 {
            led.hours_granted.entry(date.clone()).or_insert([0.0; 24])[hour as usize] += d_granted;
        }
        led.prune_hours(
...
///
/// 增量归入「采样时刻所属的那个小时」，所以**所有**把 `at` 映射到小时桶的地方
/// 都必须走这一个函数 —— 包括 `credit_briefing_enable` 的起点条目，
/// 否则跨整点时基线条目的归属会和账本科目错开一个小时后桶里取不到余额。
pub(crate) fn parse_at(at: &str) -> Option<(String, u8)> {
...
/// 台账文件的 schema 版本。
///
/// **对不上直接丢弃**（与简报文件同一套策略）。从 1 升到 2 的原因是记账换组了：
/// 旧文件里存的是 `Capacity*` 家族的最大值（那一组接口根本不维护、多为 0），
/// 若拿它当 `Cycle*` 的基线，换组后的第一次采样就会把「0 → 本周期已用」整笔算成
/// 某一个小时的消耗 —— 那是纯粹的假账。本项目的适配版直接从累计量（credits_limit /
/// credits_amount）记账，沿用 2 这个版本号。
///
/// 从 2 升到 3 的原因是**逐包键换了口径**（`描述@到期` → `entitlement_id`，见
/// [`PkgView::key`]）。不丢旧文件的话，新键在旧账里一个都找不到 ⇒ 每个包都被当成
/// 「新包」入账，那**一整个装机以来的已用量与授予量会当场算成某一个小时的消耗与新增**。
/// 丢弃之后首次采样只对齐基线（[`Mode`] 的既有语义），代价只是那一小时的明细，
/// 而简报里已经固化好的时条目不受影响（它存的是账号级聚合，不含包键）。
...
mod tests {
    ...
    /// 回归：**首采样拿到空视图**（接口失败 / 账号一个包都没有）时不能置 `seeded`。
    #[test]
    ...
    /// 回归：**键必须真的逐包** —— 同一批「老用户福利」里有两个包，描述与到期完全一样、
    /// 只有 `entitlement_id` 不同，其中一个已经用满、另一个还在消耗。
    ///
    /// 键若退化成「描述@到期」，两者会并成一条，`max` 把这条钉在用满的那个（used=2000）上，
    /// 另一个从 1288.93 继续涨的增量永远记不进来 —— 真机症状正是「余额在掉、简报一条不涨」。
    #[test]
    fn same_name_packages_must_be_keyed_apart_or_their_consumption_is_lost() {
        let both = |used_a: f64, used_b: f64| {
            vec![
                pkg("357880625666", 2000.0, used_a, ""),
                pkg("357880625410", 2000.0, used_b, ""),
            ]
        };

        let mut led = Ledger::default();
        sample(&mut led, "a1", &both(2000.0, 1288.93), "2026-09-21 09:00:00");
        sample(&mut led, "a1", &both(2000.0, 1321.93), "2026-09-21 10:00:00");
        assert_eq!(
            day(&led, "a1", "2026-09-21")[10],
            33.0,
            "用满的那个包不该吃掉另一个包的增量"
        );

        // 对照：键退化成一个（老口径）时，这一笔消耗被那个用满的包整个吃掉
        let mut same = Ledger::default();
        sample(&mut same, "a1", &[pkg("老用户福利@1791979006000", 2000.0, 2000.0, "")], "2026-09-21 09:00:00");
        sample(&mut same, "a1", &[pkg("老用户福利@1791979006000", 2000.0, 1321.93, "")], "2026-09-21 10:00:00");
        assert_eq!(
            day(&same, "a1", "2026-09-21")[10],
            0.0,
            "同一个键合并取最大值 ⇒ 增量恒为 0，这就是那个 bug"
        );
    }
    ...
    fn the_store_takes_a_whole_batch_in_one_go() {
        let dir = std::env::temp_dir().join(format!("twa-ledger-store-{}", uuid::Uuid::new_v4()));
    ...
    fn ledger_round_trips_and_tolerates_a_missing_file() {
        let dir = std::env::temp_dir().join(format!("twa-ledger-file-{}", uuid::Uuid::new_v4()));
