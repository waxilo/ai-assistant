...
/// 一键刷新全部账号：**拉一次 `get-user-resource`，把结果写进唯一的数据来源（积分台账）**。
///
/// 1. **积分事实**：剩余积分 + 最早过期时间 + 逐包明细 → 全部交给台账（见 [`crate::ledger`]）。
///    账户列表的「剩余积分」列、智能接管路由、积分简报因此读的都是同一份数；
/// 2. **签到状态**：只读查询「今日是否已签到」（不打签到接口、零副作用），写入 `checked_today`。
///
...
pub async fn refresh_all(app: AppHandle) -> Result<Vec<accounts::AccountView>, String> {
    ...
    for i in 0..accounts.len() {
        ...
        // 2) 签到状态：只读查询今日是否已签（持久化 checked_today）
...
/// 采集阶段：逐账号打一次 `get-user-resource`，把读数攒起来 —— **不碰台账**。
///
