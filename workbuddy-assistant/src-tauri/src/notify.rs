...
/// 否则会变成「【WorkBuddy 助手】WorkBuddy 签到完成」这种重复）。
pub const BRAND: &str = "【WorkBuddy 助手】";
...
mod tests {
    ...
    fn brands_every_message_exactly_once() {
        assert_eq!(branded("签到完成"), "【WorkBuddy 助手】签到完成");
        // 幂等：调用点万一自己写了前缀，也不该变成两个
        assert_eq!(
            branded("【WorkBuddy 助手】签到完成"),
            "【WorkBuddy 助手】签到完成"
        );
    ...
    fn brand_reaches_the_webhook_query() {
        ...
        let msg = parsed
            ...
            .map(|(_, v)| v.to_string())
            .unwrap();
        assert!(msg.starts_with("【WorkBuddy 助手】"), "{msg}");
