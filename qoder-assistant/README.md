...
│       ├── patch.rs        # 接管注入补丁器：改客户端 app.asar.unpacked 里所有真实存在的 worker 产物副本（端点 + 本地 CA + 指纹自愈 + 精确剥离；每次会话重读、无需重启）
...
3. **官方更新会覆盖它 → 指纹自愈。** 每次心跳（5s）比对文件头：不是我们的注入段就重打一遍。
   ...
   不会出现「还原把用户刚更新的客户端降级」。
   ⚠️ 重打**也只改磁盘**：改动在下一次对话被客户端读到，无需任何进程操作。
4. **摘除是精确剥离，不是回滚备份。** 关闭开关 = 把注入段逐字节剥掉、还原成官方原文
   （有单测逐字节比对），而不是拿备份覆盖 —— 备份可能已经不是当前那版了。
   摘除同样**只改磁盘**：客户端在下一次对话重读产物，自然回到直连。
5. **不重启任何客户端。** 客户端**每次会话**都新起 worker 线程、从磁盘重读产物
   （2026-09-24 惰性标记活体实验终审，CN / 国际两变体都实测命中），所以开关接管 /
   换端口 / 换区域都不碰任何进程 —— 改动在下一次对话自然生效。这里曾有「自动优雅
   重启客户端」的收尾（2026-09-21~09-23 的 `client_proc`），它建立在对旧版（≤0.3.3）
   独立长驻子进程架构的观察上，已随架构淘汰整体删除。本应用退出时更不会去动进程。
6. **换区域 = 换一个客户端接管。** 装新区域前先按旧租约把旧区域的注入卸干净，否则旧客户端
   会一直指向一个已经没人监听的端口；两个区域的产物各自改写，各自在下一次对话生效。
...
> 3. 现在改的是**客户端每次会话都要执行的那份产物本身**：在 `app.asar.unpacked` 里的 worker
>    产物头部注入端点与本地 CA（见本节开头）。它不依赖任何配置文件、也不改系统信任库，
>    改动在**下一次对话**被读到 —— 无需重启（2026-09-24 惰性标记活体实验终审；
>    09-21 曾认定「必须重启」，那是旧版长驻子进程架构的观察，该架构已不存在）。
>    代价是官方更新会覆盖 → 靠心跳比对文件头指纹自愈（重打同样在下一次对话生效）；
>    摘除是**精确剥离**，不是拿备份回滚。
>    ⚠️ 还有一层：客户端 fast-update 之后，安装根里会同时存在顶层原件与
>    `.qoder-versions/<版本>/` 里的新副本，而应用跑的是**版本目录**那份 ——
>    所以注入是**全量**的（`patch::install_everywhere`），漏一份就可能「打在没人读的文件上」。
>
...
- **接管不碰任何客户端进程**（2026-09-24 起，原 `client_proc.rs` 已删除）：
  客户端每次会话从磁盘重读 worker 产物，开关接管 / 换端口 / 换区域都只改磁盘，
  下一次对话自然生效。历史上曾有「优雅退出 + 重新拉起」的自动重启
  （macOS `osascript quit` + `open`；Windows `taskkill /im` 不带 `/f`、不带 `/t`），
  它的三条硬规矩与两平台差异记录在 `basedata/` 的过程文档与 git 历史里。
