...
- **多账号管理**：账号条目**只读展示**（名称 + 展示标识 + 凭证有效期 + 剩余积分），不做手工录入与编辑 —— 凭证一律来自「登录新账号」或「导入本机账号」，避免粘贴错 token。删除账号时会一并清理它的签到日志。
  - **展示标识按区域选**：国内版看手机号、国际版看邮箱（另一项作回退；规则与后端
    `Account::identity` 同源，界面、签到日志、简报弹窗与推送明细四处一致）。
  ...
  - 邮箱同样两条路都拿得到：登录文件的 `user.email`、`userinfo` 的 `email`；老账号在下一次
    刷新时补一次（只补空值，不覆盖已有）。它也是导入合并的识别键之一 —— 续签轮换过 token 的
    账号靠它并回同一条，而不是新增一个「同人不同 token」的条目。绑定了云端凭证池时它还**随池搬运**
    （`PoolItem.email`）：别的机器收养这条账号当场就显示得出邮箱，不必自己再刷一次。
- **后台常驻（系统托盘）**：点窗口关闭按钮 = 隐藏到系统托盘，**进程不退出**（定时签到、token 续签、本地反代持续生效）；托盘菜单提供「显示主窗口 / 退出」，macOS 点 Dock 图标也会唤回主窗口。再点一次应用图标不会另开一份（单实例守卫，Windows 上尤其明显），而是唤回已有窗口。真正退出请走托盘菜单「退出」（退出时会自动关闭智能接管：把客户端产物还原成官方原样，**但不动正在运行的 Qoder** —— 它下次自己启动时读到的就是干净的产物）。
...
- **定时自动签到**：每天在设定时刻（默认 `10:00`，对齐活动刷新点）自动跑一遍「全部签到」，**国际版与国内版一起签**（两边的每日活动窗口本来就是同一个：都是 10:00 UTC+8 开到次日 09:59，所以统一的触发时刻对哪边都不算早），**应用运行期间生效**；错过时刻后 30 分钟内打开应用会自动补签一次，跨启动不会重复签（`schedule_state.json` 记录已执行日期，且**每个区域都记**——当天切换区域不会再按那边那份排期签一遍）。配套提供「开机自启动」开关，让定时签到真正能每天生效。
...
- **智能接管（Qoder 专用反代）**：在 `127.0.0.1:8789`（可改端口；避开同机 workbuddy-assistant 占用的 8787）起一个 Qoder 专用反代，开启后自动把 Qoder 的对话请求接管到本地——只在**勾选的扣费备选账号**里选号（未勾选的不允许扣费，全不勾 = 全部可用；会话粘滞 + 积分最早过期优先轮换）。开启 / 关闭 / 换端口 / 换区域时，**若 Qoder 正在运行，本应用会优雅重启它**（产物改动要客户端重启才被读到，见下），没在运行就等你下次打开。页面下方是**接管动态**：只列对客通知（开启 / 关闭接管、**这轮对话由哪个账号提供**、限流切换、重启结果、异常），请求级细节落在调试日志、界面上「查看日志」一键定位。详见 [智能接管](#智能接管qoder-专用)。
- **账号获取（两条通道，无手工录入）**：
  - **导入本机账号**：直接读 Qoder 写在本机的凭据文件 `auth.v1.dat`（Chromium `safeStorage` / **OSCrypt** 加密，**两个平台都能解**：Windows 走 DPAPI、macOS 走系统钥匙串），**不需要应用运行、也不需要调试端口**，一次就能拿到 token + 昵称 + 手机号 + 邮箱 + refresh token（已存在的账号会合并补全凭证，不会重复添加）。
  - **登录新账号**：走 Qoder **设备授权流**（`/device/selectAccounts` → 浏览器扫码 → 轮询 `/api/v1/deviceToken/poll`），**不重启、不打断当前 Qoder、不改动本机登录文件**，能主动签发**任意新账号**的凭证、昵称、手机号与邮箱（与「导入本机账号」互补：后者只能收编已经登录过的那个）。
...
- **资源包到期是「三态」**，界面上分别说：有到期日（`N 天后过期` / `已过期`）· 服务端明说不过期（`不过期`）· 响应里没给到期信息（未知，退回「查看资源包」）。
  - 附加额度（签到送的那 100 Credits）的到期时间**不在额度接口里**：`qoderUsage.expiresAt` 是整个额度概览的到期（= 计划周期终点，免费号还是「无期限」哨兵），旧版把它同时当成两个槽位的到期，于是界面对免费号显示「不过期」。真实日期来自**发放凭据** —— 领取接口 `POST …/{campaignId}/claim` 响应里的 `expiresAt`（实测 `claimedAt + 30 天`，见 `qoder_api::ClaimReceipt`）。已领取的活动**幂等回放**（`replayed: true`）同样返回这张凭据，所以签到路径每天顺手把这一笔落进台账（`ledger::note_grant`），启动时再按 `checkin_logs.json` 把历史补一遍（幂等：按到期时刻去重）。
  - 聚合包（「附加额度」把每天领的 100 分合成一格）的「到期」取**最早一笔未过期的发放**，不是最后一笔 —— 资源包弹窗（账号页「N 天后过期」点开）里同时逐笔列出量 + 到期日，几笔、各哪天作废一目了然；接管路由「积分最早过期优先」用的也是这个值。
...
│   └── src/
│       ├── main.rs / lib.rs
│       ├── accounts.rs     # 多账号 JSON 存储（含 phone / email；文件权限 0600）
│       ├── auth_file.rs    # 读本机 Qoder 登录文件（含昵称/手机号/邮箱）
...
│       ├── client_proc.rs  # 客户端进程操作：探活 / 优雅退出 / 重新拉起（产物要重启才被读到，开关接管的收尾）
│       ├── proc.rs         # 辅助进程的统一入口（唯一目的：Windows 上不弹黑色命令框）
...
2. **key 会漂移**：上传时账号还没补上手机号（手机号 / 邮箱是导入后 `fill_identity_if_missing` 补的），
...
展示标识（`phone` / `email`）只随池**携带**，不进上面任何一条判据：客户端按区域选首选、另一项
回落来显示，而邮箱一旦进了 `key` 反而会让老池的 key 漂一次（且「一台补过邮箱、另一台还没有」的
两台机器会来回改写池里那条）。所以它是**纯携带**：`to_item` 带上、`account_from_item` / `adopt`
按「补空值、不覆盖」落地。

服务端侧同步修复：`normalizeItem` 必须原样往返 `region`（缺省时按 key 前缀回填）与展示标识
（`phone` / `email`，老条目缺这个键 → 空串），并且**读出口也走一遍规范化** —— D1 的 `payload`
是不透明 JSON，老条目在读的那一刻就自愈。

---

## 定时自动签到

在「设置」里开启「每天定时自动签到全部账号」并选好时刻（默认 `10:00`，对齐服务端的
活动刷新点：当天那条 10:00 UTC+8 生成、次日 09:59 关闭，落在刷新前的时刻永远领不到）。

- 实现是 Rust 侧一个**后台线程**：每 30s 读一次配置，命中就调用与「全部签到」完全相同的逻辑
  （只是范围为**跨区域**，见下条），并按设置推送通知。改了时刻/开关**无需重启应用**即生效。
- **范围是两套部署的全部账号**：国内版按 10:00 UTC+8 刷新当天活动，国际版实测那条
  `act-20260921-308` 的窗口是 `02:00 UTC → 次日 01:59 UTC`（换算成 UTC+8 同样是
  10:00 → 次日 09:59）—— 两边本来就是同一个窗口，所以一个触发时刻对哪边都不算早。
  早先「另一区域的账号有自己的刷新点，跟着这批签没意义」的结论是错的（那是把国内版的
  刷新点当成了全局事实）；手动「全部签到」则仍只签当前区域：那是「把界面上这一屏签完」。
...
- 跨启动去重：执行日期记在应用数据目录的 `schedule_state.json`，重启应用不会在补跑窗口内重复签。
  记账**按整机算**：触发一次就把每个区域都记上「今天跑过了」，所以当天切换区域也不会
  按另一边那份排期再签一遍（排期的时刻/时间窗仍各随各区域保存）。
...
- 推送内容形如：`【Qoder 助手】签到完成：成功 2 / 已签 1 / 失败 1（共 4 个账号）`，
  有失败时附上前 5 条「账号名（展示标识）：失败原因」明细（国内版手机号 / 国际版邮箱，
  与界面同一套规则）——这才是推送里最有价值的信息。
...
```bash
# 请求链路（已落地）：
# Qoder 桌面端**启动时**读一次 worker 产物（端点与 CA 就写在那里面，见下）
#            → 每次会话 spawn 一次性 `--print` 推理进程（跑完即退，没有长驻 host）
...
# 它由桌面端在 spawn 时从**启动时读进内存的那份产物**构造；客户端把 scheme 写死成 https，
# 所以反代必须真的能完成 TLS 握手。
# ⚠️ 产物只在客户端启动时被读一次 —— **改完必须重启 Qoder 才生效**（开关接管时本应用会自动重启它）。
```
...
1. **注入点 = 真正被执行的那个文件。** Qoder 每次会话起的一次性进程，argv 指向
   ...
   桌面端**启动时**把这个文件读进内存，之后每次会话都用内存里那份副本 ——
   这也正是「改完要重启客户端」的来源。
...
3. **官方更新会覆盖它 → 指纹自愈。** 每次心跳（5s）比对文件头：不是我们的注入段就重打一遍。
   ...
   不会出现「还原把用户刚更新的客户端降级」。
   ⚠️ 重打**也只改磁盘**：官方更新后已经跑起来的那台客户端内存里还是旧的一份，
   要重启它才回到接管 —— 页面上的做法是「关一下开关再开」（那一步会重启它）。
4. **摘除是精确剥离，不是回滚备份。** 关闭开关 = 把注入段逐字节剥掉、还原成官方原文
   （有单测逐字节比对），而不是拿备份覆盖 —— 备份可能已经不是当前那版了。
   摘除同样**只改磁盘**：正在运行的客户端内存里还揣着注入，得重启它才回到直连。
5. **改完谁来重启客户端：`client_proc`。** 产物要客户端重启才被读到（见第 1 条），所以
   开关接管 / 换端口 / 换区域这一步的**收尾**是：正在运行的客户端**优雅退出**（不发强杀、
   不打断未保存内容的保存流程）并重新拉起；**没在运行就一个进程都不碰**。三条结局
   （真重启了 / 它没在运行 / 拉不起来）都如实写进接管动态 —— 重启失败不算开关失败，
   产物与反代都已就位，只是要用户手动重启一次。本应用**退出时不做这件事**：
   退出路径服务的是下一次启动，为此关掉用户正开着的编辑器是过分的打扰。
6. **换区域 = 换一个客户端接管。** 装新区域前先按旧租约把旧区域的注入卸干净，否则旧客户端
   会一直指向一个已经没人监听的端口；两个区域的客户端都要重启一遍才各自生效。
...
- **Qoder 专用**：只监听 `127.0.0.1`、无鉴权 Key（不对外提供通用代理能力）；
  ...
  进程层面的规矩：**开关 / 换端口 / 换区域**这一步会把正在运行的客户端优雅重启一次
  （产物要客户端重启才被读到），**应用退出时不动它** —— 见本节末尾的机制订正。
- **接管目标区域**：控制条上的「区域」选择器决定三件事 —— 端点注入哪套客户端的产物、
  请求转发到哪个模型网关、以及扣费账号**从哪个池里选**（跨区域的 token 在对方网关上无效，
  所以扣费池与模型清单都只列该区域）。开启期间换区域会走安全切换流程
  （摘旧区域的注入 → 装进新区域 → **两个客户端各自重启一次**），并把扣费池重置为「全部」；
  关闭期间换区域只是把设置存下来（那时没有任何注入，也就没什么要重启的）。
...
- **网络急救**：设置 →「一键诊断 / 一键恢复」。会扫描配置文件、launchd 全局变量、
  ...
  （原先还有一条「桌面端仍持有已摘除端点」的判定，曾以「没有长驻 host」为由删掉 ——
  那个理由**只对了一半**（见下面的更正），但这条判定不需要回来：现在「一键恢复」会**顺手把
  刚被摘掉注入的客户端重启一遍**，同一个事实由动作而不是由告警处理。配置文件里的
  `CODEBUDDY_*` 端点键现在**只剩历史残留价值** —— Qoder 不读它们，所以清它属于清理，
  不是修复连通性。真正会「断网」的是**产物里留着指向本机端口、而反代已经不在**的僵尸注入，
  诊断会把它单独报成 `block` —— 详见 `netfix.rs` 模块文档。）

> **关键机制（2026-09-21 订正）**：
>
> 1. 2026-09-19 的更正里有一半是错的，先纠正这一半：「桌面端与其长驻 CLI host 只在启动时
>    读一次端点配置，所以开关接管要自动重启客户端」——**「长驻 CLI host」确实不存在**
>    （Qoder 的推理进程是每次会话按需 spawn 的一次性 `--print` 进程，证据：
>    `~/.qoder[-cn]/logs/runs/<时间戳>-p<桌面端pid>/manifest.json` 的 `argv`，跑完即退），
>    但「**要重启客户端**」这个结论是对的：桌面端自己是长驻进程，worker 产物在
>    **首次启动时**被读进内存，之后每次会话都用内存里那一份 —— 磁盘改了它不看。
>    **实测**：开启接管后不重启 Qoder，对话仍然直连官方。
> 2. 另两半没变：那个一次性进程的 env 由**桌面端在 spawn 时构造**，而 `~/.qoder[-cn]/settings.json`
>    的 `env` 块**没有任何消费者** —— 所以旧实现写的端点键从来没被读过，接管是**空转**。
>    所以「改配置就等于生效」不成立，「改配置文件」也不是落点。
> 3. 现在改的是**那个一次性进程要执行的那份产物本身**：在 `app.asar.unpacked` 里的 worker
>    产物头部注入端点与本地 CA（见本节开头）。它不依赖任何配置文件、也不改系统信任库，
>    但**改完必须重启客户端才会被读到** —— 这一步由 `client_proc` 在开关接管 / 换端口 /
>    换区域时自动完成（正在运行才重启、优雅退出、三种结局都记进接管动态）。
>    代价是官方更新会覆盖 → 靠心跳比对文件头指纹自愈（重打同样要重启才被读到）；
>    摘除是**精确剥离**，不是拿备份回滚。
>
> 旧的进程操作确实留下过一个真实故障：`open -a` 紧跟在 AppleScript `quit` 之后
...
> 要么全成、要么整体回滚（落盘 → 等端点就位 → 失败则连设置一起退回）；重启是**收尾**，
> 它失败只如实记一条动态，不把已经就位的拓扑拖回去。
...
- **接管时的「重启 Qoder」两个平台怎么做的**（`client_proc.rs`）：
  - macOS：`osascript -e 'quit app "<.app 路径>"'` 请求正常退出 → 等它退干净 → `open "<.app 路径>"` 重新拉起。
    用**路径**而不是应用名，免得被 `~/Applications` 里的同名副本或 LaunchServices 解析抢走。
  - Windows：`taskkill /im "<exe>"`（**不带 `/f`**：这是编辑器，强杀会丢未保存内容；
    **也不带 `/t`**：Chromium 系那一堆无窗口子进程收不到关闭消息，带了 `/t` 反而一个都退不掉）
    → 等它退干净 → 直接 spawn 安装目录里的 exe。
  - 两条硬规矩：`spawn()` 之后**绝不 `wait()`**（等的是「它有没有出现」，不是「它何时退出」）；
    三种结局（真重启了 / 它没在运行 / 拉不起来）都如实记进接管动态，绝不合成一句「已处理」。
...
| `import_accounts` | 批量导入账号（「导入本机账号」/「登录新账号」共用）：按「**区域 + token / 手机号 / 邮箱**」识别后**合并补全**凭证，不会重复添加 |
| `remove_account` | 删除账号（同时清理该账号的签到日志） |
| `checkin_one` | 对单个账号签到 |
| `checkin_all` | 批量签到：手动按钮只签**当前区域**；`allRegions = true` 时跨区域全签（「启动即签到」与定时签到走它，见 [定时自动签到](#定时自动签到)） |
| `refresh_all` | 一键刷新：重拉并持久化**当前区域**账号的积分快照 / 签到状态 / 积分余量 |
| `discover_local_accounts` | 读取本机 Qoder 登录文件（两个 profile 目录都扫；含区域/昵称/手机号/邮箱/有效期） |
...
| `apply_settings` | 原子应用设置；接管启停 / 换端口 / **换区域**时走安全切换流程（摘端点 + 装到新区域，并**自动重启正在运行的那个客户端**让改动被读到）；任一步失败整体回滚。重启是收尾：它失败只记一条接管动态，不回滚拓扑 |
...
| `net_diagnose` / `net_restore` / `reveal_path` | 网络急救：只读诊断 / 一键恢复（自动备份被改文件，清理两个区域的残留端点，并把**刚被摘掉注入**的那个客户端重启一遍）/ 在文件管理器里定位 |
