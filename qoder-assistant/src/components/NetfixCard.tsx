 ...
 * 「网络急救」卡片——嵌在设置页里。
 *
 * 它排查的是 Qoder 的**全局**网络残留配置（settings.json 的 env、launchd 全局环境变量），
...
export function NetfixCard({
  ...
  const doRestore = async () => {
    const ok = await askConfirm({
      title: "一键恢复网络配置",
      body:
        "会依次：关闭智能接管、把被改过的客户端产物还原成官方原样、" +
        "备份并清除全局调试端点、取消 launchd 全局环境变量，最后复检。" +
        "接管过的客户端若正在运行，会被**自动优雅重启**一次（否则它内存里仍是旧端点，" +
        "恢复不会生效）—— 请先保存好未保存的内容。被改动的文件都会先备份。继续？",
      okText: "恢复",
      danger: true,
    });
    if (!ok) return;
    ...
    <div className="set-card card">
      ...
      <div className="set-group">
        <Row
          title="网络链路诊断"
          desc={
            "调试「自定义服务端点」时，如果把地址写进了 Qoder 的全局配置" +
            "（~/.qoder/settings.json 的 env，或 launchd 全局环境变量），" +
            "受影响的会是整个 Qoder，典型表现是「502 连接被拒绝」。"
          }
          ctrl={
            <>
              <button
            ...
            {netReport.healthy && netReport.issues.length === 0 ? (
              <div className="status-banner ok">
                ...
                <div>
                  ...
                  <span>未发现残留端点配置，Qoder 的网络链路是干净的。</span>
                </div>
              </div>
            ) : (
              <>
                <div className="status-banner warn">
                  ...
                  <div>
                    ...
                    <span>以下项可能影响 Qoder 网络连接，建议点「一键恢复」清除。</span>
                  </div>
                </div>
