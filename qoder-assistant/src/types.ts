...
export interface ModelInfo {
  ...
  free: boolean;
  /** 积分倍率原始串（如 "x0.2" / "x0"），仅展示用；空串 = 倍率未知 */
  multiplier: string;
  /**
   * **折扣前**的倍率（错峰促销期间目录给的原价，如 "x0.5"）；空串 = 没有折扣。
   *
   * 界面画成「~~x0.5~~ x0.2」：只有倍率而没有原价，用户看不出这是活动价还是常价。
   */
...
export interface ModelReport {
   ...
   * 后端给的是成句的中文，界面不要自己拼（「这个区域还没账号」与「有账号但这次没拉到」
   * 的下一步动作完全不同：前者去登录，后者该看网络或登录态）。
   */
  note: string | null;
}
