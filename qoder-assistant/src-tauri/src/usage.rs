...
//!
//! 控制台的用量页有一条能列出**每个资源包各自到期**的接口：
//! `GET https://qoder.cn/api/v1/me/usages/big_model_credits` →
//! `resource_package_quotas[]: {remaining_value, expires_at, source: …_BONUS}`。
//! 它比我们的凭据路子更全（能覆盖「在别的机器 / 网页上领的那几笔」），但**只认浏览器
//! 会话 cookie**：同一条请求换成我们的 `Bearer dt-…` 是 `401 User not authenticated`，
//! 换成自签 COSY 也是 `401`，而 sash 网关上这个路径干脆 `404`
//! （只有它的子路径 `/histories` 回 `400`，那是**消耗流水**、里面没有到期字段）。
//!
//! 好在也不需要它：**同一个账号**（那次对照的浏览器 session 就是台账里那个 13:16 领取的
//! 账号）两边给的到期时刻是 `1792386992580`（网页 `expires_at`）与 `1792386993580`
//! （我们的领取凭据 `expiresAt`）—— 同一批积分，差 1 秒，是服务端两处写入的抖动。
//! 所以「附加额度 到期 未知」从来不是缺接口，而是那笔领取发生在我们还没记录凭据之前
//! —— 下一次签到（幂等回放同样回 `expiresAt`）就会补上。
//!
//! # 与台账（`ledger`）的契约
//!
