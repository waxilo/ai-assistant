//! Qoder 全局网络配置的**诊断**与**一键恢复**。
//!
//! 背景：调试本地反代时，如果把「自定义服务端点」写到 Qoder 的**全局**配置里，
//! 受影响的不是某个子进程，而是**整个 Qoder**（含正在运行的桌面端）——表现为
...
//!
//! 之所以自己拼 JSON 而不是复用 `Settings`：被扫的是 **Qoder 自己的**配置文件，
...
//! 是**桌面端自己**。
//!
//! # 2026-09-21 修正（其结论已于 2026-09-24 再次修正）：僵尸注入的影响面
//!
//! 上面把僵尸注入定义成「产物里留着本机端点、而反代已经不在」—— 那只覆盖了磁盘这一半。
//! 09-21 曾认为另一半是「产物干净了、运行中的客户端还揣着脏的那一份，要重启才清掉」，
//! 并为此在恢复流程里补了「重启刚被摘掉注入的客户端」一步。
//!
//! **2026-09-24 惰性标记活体实验推翻了那一半**：0.3.4 起的客户端每次会话都新起
//! worker 线程、从磁盘重读产物（CN / 国际两变体都是，run manifest 的 pid = 桌面主进程
//! pid），所以摘干净磁盘之后，运行中的客户端在**下一次对话**就回到直连 ——
//! 重启步骤整体删除。09-21 观察到的「不重启就直连」描述的是旧版（≤0.3.3）独立
//! 长驻子进程架构，且当时多半混入了「补丁打在没人读的副本上」的干扰（见
//! [`crate::patch`] 的全量注入）。真正的僵尸态只剩磁盘那一半，`takeover_issues`
//! 读产物判定即可。
//!
...
/// `~/.qoder/settings.json` → `~/.qoder/settings.json`（家目录缩写成 `~`）
...
fn judge_value(value: &str) -> (String, String) {
    ...
    if tcp_alive(&host, port) {
        (
            "warn".into(),
            format!(
                "指向本机 {host}:{port}，该端口当前「有服务在听」。Qoder 的流量会被引到本地，\
                 不在该端口时就会断网；确认不需要就清掉。"
            ),
        )
    } else {
...
/// 扫一遍所有污染点。**只读**。
///
/// `home` 用于定位 Qoder 的全局配置；`data_dir` 是本应用自己的数据目录，
/// 用来读它的反代开关。
        ...
        if let Some(env) = v.get("env").and_then(Value::as_object) {
            for key in ENV_KEYS {
                ...
                issues.push(NetIssue {
                    ...
                    value: truncate(val, 120),
                    level,
                    // 这个键**会**被整个 Qoder 进程继承（含桌面端），是真凶
                    note: format!("{note}（写在这里会被整个 Qoder 进程继承，含正在运行的桌面端。）"),
                    fixable: true,
                });
            }
        }
    ...
    if settings.proxy_enabled {
        ...
        if !alive_takeover {
            ...
            issues.push(NetIssue {
                ...
                level: "warn".into(),
                note: "反代本身不会导致 Qoder 断网。若你正在排查「网络不通」，建议一并关掉以排除干扰。"
                    .into(),
                fixable: true,
            });
        }
...
///
/// 这里曾有第三个开关 `touch_processes`（「重启刚被摘掉注入的客户端」），
/// 2026-09-24 随自动重启机制一起删除：客户端每次会话重读产物，摘干净磁盘
/// 之后它下一次对话就回到直连，进程上没有任何要收尾的东西。
/// 单测传 `false` 只是为了别去动 launchctl 的整机状态。
fn restore_impl(home: &Path, data_dir: &Path, touch_launchd: bool) -> NetRestoreReport {
    ...
    //    摘干净之后，运行中的客户端在下一次对话重读磁盘、自然回到直连 ——
    //    这里曾有一步「重启刚被摘掉注入的客户端」，2026-09-24 已删（见模块文档）。
        ...
        Ok(()) => {
            if crate::stealth::load_lease(data_dir).is_none() {
                steps.push(NetStep {
                    action: "摘除接管端点".into(),
                    ok: true,
                    detail: "客户端 worker 产物已还原成官方原样，租约已清除；\
                             正在运行的客户端下一次对话起恢复直连。"
                        .into(),
                });
            }
...
fn home_dir() -> Result<PathBuf, String> {
    dirs::home_dir().ok_or_else(|| "无法定位家目录，无法诊断 Qoder 全局配置。".to_string())
}
...
/// 走线程池（`command(async)`）而不是主线程：这条流程可能经
/// [`crate::commands::apply_settings_inner`] 等端点就位（最长 8 秒）。
/// 堵在主线程上会让整个窗口「未响应」。
#[tauri::command(async)]
...
mod tests {
    ...
    fn diagnose_flags_pollution_and_restore_cleans_it() {
        ...
        let cfg = home.join(".qoder").join("settings.json");
        fs::write(
            &cfg,
            r#"{
